<template>
  ...
  <div class="user-profile">
    ...
    <div class="user-details mt-4 text-center">
      ...
      <div class="card-text">
        ...
        <div class="d-flex">
          <input v-bind:type="passwordFieldType" v-if="Korisnik.editing" v-model="Korisnik.lozinka"
      ...
      <div class="card-text">
        ...
        <div class="d-flex">
          <input v-bind:type="passwordFieldType" v-if="Korisnik.editing" v-model="Korisnik.nova_lozinka"
            class="form-control text-center">
          <button v-if="Korisnik.editing" class="btn btn-link" @click="togglePasswordVisibility">
            <i v-bind:class="passwordFieldType === 'password' ? 'far fa-eye' : 'far fa-eye-slash'"></i>
          </button>
      ...
      <div class="card-text">
        ...
        <div class="d-flex">
          <input v-bind:type="passwordFieldType" v-if="Korisnik.editing" v-model="Korisnik.potvrda_lozinke"
            class="form-control text-center">
          <button v-if="Korisnik.editing" class="btn btn-link" @click="togglePasswordVisibility">
            <i v-bind:class="passwordFieldType === 'password' ? 'far fa-eye' : 'far fa-eye-slash'"></i>
          </button>
  ...
  <button class="btn btn-dark" @click="modalOpenCloseHandlerDodavanjeObjava">
    <i class="fas fa-plus"></i> Dodaj novu objavu
  </button>
  <h2 class="mt-4" v-if="showNoPostsMessage">Nemate objava</h2>
  <div v-if="otvorenModalObjava">
    <ModalObjava @close="modalOpenCloseHandlerObjava" :showEditButton="true" :objava="objava" />
  </div>
  <ModalDodajObjavu v-if="dodavanjeObjave" @close="modalOpenCloseHandlerDodavanjeObjava" :user_id="Korisnik.id"
    @potvrdjeno="UspjesnoSpremljeno" />
  <div class="card-container-objava-user">
    <div class="card card-objava-user shadow" v-for="(objava, index) in objave" :key="index"
      @click="modalOpenCloseHandlerObjava(objava)">
...
export default {
  ...
  methods: {
    modalOpenCloseHandlerDodavanjeObjava() {
    ...
    setRating(objavaIndex, rating) {
      ...
      alert("Ne možete ocjeniti svoju objavu!")
    },
    modalOpenCloseHandlerObjava(objava) {
